    ...
    .input(
      ...
      const lineItems: Stripe.Checkout.SessionCreateParams.LineItem[] =
        products.docs.map((product) => ({
          quantity: 1,
          price_data: {
            unit_amount: product.Price * 100, //Stripe does price in cents
            currency: "usd",
            product_data: {
              name: product.name,
              metadata: {
      ...
      success=true`,
        cancel_url: `${process.env.NEXT_PUBLIC_APP_URL}/tenants/${input.tenantSlug}/checkout?
      cancel=true`,
        mode: "payment",
  ...
  getProducts: baseProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const data = await ctx.db.find({
        collection: 'products',
        depth: 2, //Populate category and image, tenant & tenant.image
        where: {
          and: [
            {
              id: {
